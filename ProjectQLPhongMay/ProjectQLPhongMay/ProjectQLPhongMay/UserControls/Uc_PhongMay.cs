...
namespace ProjectQLPhongMay
{
    public partial class Uc_PhongMay : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        {
            int soluong = int.Parse(edtSLThucTe.Text);
            if (soluong > 0)
            {
                XtraMessageBox.Show("Không được xóa phòng máy có số lượng máy thực tế > 0", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
